...
- **Zwift workouts** (`training/zwift/`, served at `im.burkeruder.ai/zwift`, added 2026-09-22 at Burke's ask): every ride on PLAN.md's bike menu as a Zwift custom workout (.zwo), generated by `node training/zwift/build.js` from the definitions in that file. Power targets are %FTP; Burke's Zwift FTP should be 133 W until the 20-min test replaces it. **To add or change a workout:** edit `build.js`, run it, add the new slug to the import table in `worker/zwift.js`, `wrangler deploy`. When posting a bike session to `sessions`, name the workout in `planned_desc` so the orders card tells him which file to pick. Zwift has no public API — files reach his devices by him dropping them in `Documents/Zwift/Workouts/<ZwiftID>/` on a Mac/PC (they cloud-sync to iOS/Apple TV from there). The hands-off upgrade, if he wants it later, is intervals.icu → Zwift's Training API (free tier pushes planned workouts into Zwift's planner); not built.
